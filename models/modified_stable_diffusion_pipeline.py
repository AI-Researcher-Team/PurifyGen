...
class ModifiedStableDiffusionPipeline(StableDiffusionPipeline):
    ...
    def _new_encode_positive_prompt_space(self, negative_prompt_space, max_length, num_images_per_prompt, pooler_output=True):
        device = self._execution_device
    ...
    def __call__(
        ...
        positive_prompt_space: Optional[Union[str, List[str]]] = None,
        ...
        positive_text_embeddings = self._new_encode_positive_prompt_space(positive_prompt_space, 77, num_images_per_prompt)
        project_matrix_safe = projection_matrix(positive_text_embeddings.T)
        positive_text_embeddings_new = compute_projection_numpy(positive_text_embeddings)
        sf["safree"] = True
        if sf["safree"]:
            ...
            rescaled_text_embeddings = safree_projection(text_embeddings, positive_text_embeddings_new, masked_embs,
        ...
        sf["svf"] = True
        if sf["svf"]:
            proj_ort = projection_and_orthogonal(text_embeddings, positive_text_embeddings_new, masked_project_matrix, project_matrix)
